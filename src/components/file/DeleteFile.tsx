...
const DeleteFile = ({ file, concernedFiles, onClosePopover }: DeleteFileProps): JSX.Element => {
	...
	const deleteFile = async () => {
		...
		if (user.account) {
			...
			if (deleted.success) {
				const removed = await user.fullContact.contact.deleteFiles([file.id], concernedFiles);
	...
	const moveToBin = async (deletedAt: number) => {
		setIsLoading(true);
		if (user.account) {
			const moved = await user.fullContact.contact.moveFileToBin(file, deletedAt, concernedFiles);
